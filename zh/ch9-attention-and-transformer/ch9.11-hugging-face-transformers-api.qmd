...
open-graph:
  title: ch9.11-transformer-architectures
  description: "9.11 Hugging Face Transformers API: Mapping Structures to Calls"
title: "9.11 Hugging Face Transformers API：从结构到调用"
order: 11
---

[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jshn9515/dnnl-notebooks/blob/main/zh/ch9-attention-and-transformer/ch9.11-hugging-face-transformers-api.ipynb){fig-align="left"}
...
![图 9.11.1：Hugging Face Transformers 的基本调用流程](figures/ch9.11-transformers-pipeline.svg)
...
### 9.11.1.1 AutoTokenizer：从文本到 Token IDs
...
### 9.11.1.2 AutoModel：Transformer 的主体
...
### 9.11.1.3 ModelOutput：为什么不是普通 Tuple
...
### 9.11.2.1 SequenceClassification：Encoder + 分类头
...
### 9.11.2.2 TokenClassification：每个 Token 一个预测
...
### 9.11.2.3 一个完整的 Encoder-only 示例
...
### 9.11.3.1 CausalLM：Decoder-only 语言模型
...
### 9.11.3.3 `use_cache`：KV Cache 的接口
...
### 9.11.3.4 一个完整的 Decoder-only 示例
...
这就是 9.8 和 9.9 讲过的自回归生成和 KV cache 在实际 API 中的体现。

## 9.11.4 Encoder-Decoder 模型：输入输出序列转换
...
### 9.11.4.1 Seq2SeqLM：Encoder-Decoder 生成模型
...
### 9.11.4.2 一个完整的 Encoder-Decoder 示例
...
## 9.11.5 模型内部信息：Attention 与 Hidden States
...
### 9.11.5.1 `output_attentions`：取出 Attention Weights
...
### 9.11.5.2 `hidden_states`：取出每一层表示
...
### 9.11.5.3 一个 Attention Visualization 的接口示例
