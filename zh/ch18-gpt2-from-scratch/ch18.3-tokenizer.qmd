...
```{mermaid}
%%| fig-height: 350px
%%| fig-cap: "图 18.3.0 MiniGPT 的数据流"

flowchart TD
    A[Given a Prefix] --> B[Run MiniGPT]
    B --> C[Take the logits at the last position]
    C --> D[Select the next token]
    D --> E[Append the new token to the prefix]
    E --> B
```
...
tokenizer = WordTokenizer.train(text)
...
tokenizer = BPETokenizer.train(text, vocab_size=20)
...
char_tokenizer = CharacterTokenizer.train(text)
word_tokenizer = WordTokenizer.train(text)
bpe_tokenizer = BPETokenizer.train(text, vocab_size=20)
```
...
"""
text = text.replace('\n', ' ').strip()
tokenizer = BPETokenizer.train(text, vocab_size=100)
