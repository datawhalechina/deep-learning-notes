...
当然，大多数时候，直接写 `self.weight = nn.Parameter(...)` 就够了。`register_parameter()` 更常见于参数名需要动态生成，或者你想显式控制某个名字是否注册参数的情况。例如，有些模块可以选择是否使用 bias：
...
Buffer 还有一个重要特点：当我们调用 `model.to(device)` 时，buffer 会和 parameter 一起被移动到对应设备。例如，如果我们只是把它们保存成普通属性：
...
::: {.list-table}
表 2.4.5 Parameter 和 Buffer 的区别
...
这里 `fc1`、`relu` 和 `fc2` 都是 `model` 的子模块。只要把一个 nn.Module 赋值给另一个 nn.Module 的属性，它也会被自动注册。因此，`model.parameters()` 会递归地找到所有子模块里的参数：
...
这些方法在调试模型结构、冻结部分层、替换某些子模块时都很常用。例如，我们可以找到所有线性层：
...
for name, param in lazy_linear.named_parameters():
    print(f'{name}: {param.__class__.__name__}')
```
...
for name, module in demo.named_modules():
    print(repr(name), '->', module.__class__.__name__)
```
