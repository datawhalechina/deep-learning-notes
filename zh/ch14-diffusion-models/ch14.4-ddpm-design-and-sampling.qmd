...
import dnnlpy.models.ddpm.unet as unet
...
class UNet2DModel(nn.Module):
    """Lightweight U-Net model for 2D image denoising."""
    ...
    def __init__(
        ...
        for out_ch in block_out_channels:
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        unet.ResBlock(in_ch, out_ch, time_emb_dim),
                        unet.ResBlock(out_ch, out_ch, time_emb_dim),
                        unet.AttentionBlock(out_ch),
                        unet.Downsample(out_ch) if not is_last_ch else nn.Identity(),
                    ]
                )
            )
            in_ch = out_ch
        ...
        self.mid_block1 = unet.ResBlock(last_ch, last_ch, time_emb_dim)
        self.mid_attn = unet.AttentionBlock(last_ch)
        self.mid_block2 = unet.ResBlock(last_ch, last_ch, time_emb_dim)
        ...
        for out_ch in reversed(skip_channels):
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        unet.ResBlock(in_ch + out_ch, out_ch, time_emb_dim),
                        unet.ResBlock(out_ch, out_ch, time_emb_dim),
                        unet.AttentionBlock(out_ch),
                        unet.Upsample(out_ch) if not is_first_ch else dnn.Identity(),
                    ]
                )
            )
            in_ch = out_ch

        self.final_block = unet.ConvBlock(in_ch, in_ch)
...
def denoise_v2(x0: Tensor, xt: Tensor, timestep: int, betas: Tensor) -> Tensor:
    """Remove noise from the noisy image `xt` at a given timestep using original
    image `x0`.
    """
    t = timestep
    alphas = 1.0 - betas
...
for x0, *_ in train_dl:
    ...
    loss = dF.mse_loss(pred_noise, noise)
    loss.backward()
