...
open-graph:
  title: ch19.4-mixed-precision
  description: "19.4 Mixed Precision: FP32, FP16, BF16 and Loss Scaling"
title: "19.4 Mixed Precision：FP32、FP16、BF16 与 Loss Scaling"
order: 4
---

[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jshn9515/dnnl-notebooks/blob/main/zh/ch19-llm-training-engineering/ch19.4-mixed-precision.ipynb){fig-align="left"}
...
for dtype in [torch.float64, torch.float32, torch.float16, torch.bfloat16]:
    ...
    print(f'{str(dtype)}: {total_gib:.4f} GiB')
```
...
peak_memory = dnnlpy.max_memory_allocated()
```
...
## 19.4.8 Mixed Precision 的常见误区

现在，我们总结一下关于 mixed precision 的常见误区。

1. Mixed Precision 就是把模型转成 FP16。

    不是。`model.half()` 是显式地把模型参数转换成 FP16。而 autocast 是按算子动态选择 dtype，两者不是同一件事。

2. FP16 和 BF16 都需要 GradScaler。

    通常不是。FP16 因为动态范围较小，经常需要 `GradScaler`；而 BF16 的动态范围接近 FP32，通常不需要 `GradScaler`。

3. 低精度一定让显存减半。

    不是。在 autocast 下，参数和 optimizer states 可能仍然是 FP32。主要节省的是部分激活和临时张量。

4. BF16 在任何 GPU 上都更快。

    不是。是否更快取决于硬件是否原生支持 BF16，以及实际算子是否使用了高效 kernel。

5. 用了 autocast 就不需要检查数值稳定性。

    不是。仍然应该监控 `loss`、`gradient`、训练曲线，以及是否出现 `inf` 或 `NaN`。Mixed precision 降低了数值精度，不能完全替代训练过程中的数值诊断。
...
![图 19.4.9 三种不同精度的 AMP 流程](figures/ch19.4-amp.svg){width=80%}
