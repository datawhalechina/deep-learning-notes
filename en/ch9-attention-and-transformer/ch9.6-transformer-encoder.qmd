...
open-graph:
  title: ch9.5-positional-encoding
  description: "9.6 Transformer Encoder: Stacking Self-Attention Layers"
title: "9.6 Transformer Encoder: Stacking Self-Attention Layers"
order: 6
---

[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jshn9515/dnnl-notebooks/blob/main/en/ch9-attention-and-transformer/ch9.6-transformer-encoder.ipynb){fig-align="left"}
...
## 9.6.1 Why Stack Multiple Layers of Self-Attention?
...
## 9.6.2 The Overall Structure of an Encoder Block
...
![Figure 9.6.2: Transformer Encoder Block structure diagram](figures/ch9.6-transformer-encoder-block.svg){height=600px}
...
### 9.6.2.1 First Part: Multi-Head Attention
...
### 9.6.2.2 Second Part: Residual Connection
...
### 9.6.2.3 Third Part: Layer Normalization
...
### 9.6.2.4 Fourth Part: Position-wise Feed-Forward Network
...
## 9.6.4 PyTorch Implementation of Transformer Encoder Block

Below, we implement a simplified Transformer Encoder block. To avoid repeatedly implementing multi-head attention, we directly use the `MultiheadAttention` implemented in Section 9.4.

```{python}
class TransformerEncoderLayer(nn.Module):
    """A simplified Transformer Encoder block with Pre-LN structure."""
    ...
    def __init__(
        ...
        self.self_attn = dnn.MultiheadAttention(
            d_model, num_heads, bias=bias, dropout=dropout
        )

        self.linear1 = dnn.Linear(d_model, dim_feedforward, bias=bias)
        self.dropout = dnn.Dropout(dropout)
        self.linear2 = dnn.Linear(dim_feedforward, d_model, bias=bias)
        self.activation = activation

        self.norm1 = dnn.LayerNorm(d_model, bias=bias)
        self.norm2 = dnn.LayerNorm(d_model, bias=bias)
        self.dropout1 = dnn.Dropout(dropout)
        self.dropout2 = dnn.Dropout(dropout)
...
## 9.6.5 Stacking Multiple Encoder Blocks
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.norm = dnn.LayerNorm(d_model)
...
## 9.6.6 Padding Mask: Ignoring Padded Positions
...
## 9.6.7 What Can the Encoder Output Be Used for?
