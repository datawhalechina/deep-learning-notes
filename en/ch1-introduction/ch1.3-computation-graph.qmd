...
At this point, we have seen how forward propagation produces predictions, how the loss function measures errors, and how backpropagation computes gradients for the model parameters. But gradients alone do not change the model. What actually enables the model to "learn" is the process of updating its parameters based on those gradients.

In the next section, we'll begin with gradient descent, the simplest optimization algorithm, and see how a neural network gradually adjusts its parameters to reduce the loss.
