...
```{python}
def numerical_gradient(func: Func, x: np.ndarray, eps: float = 1e-5) -> np.ndarray:
...
```{python}
def relative_error(analytic: np.ndarray, numeric: np.ndarray, eps: float = 1e-12):
    """Compute the relative error between analytic and numeric gradients."""
...
grad_W_numeric = numerical_gradient(loss_fn_weights, linear.weight.copy())
error = relative_error(grad_W_analytic, grad_W_numeric)
print('Relative Error for weight:', error)
```
...
grad_b_numeric = numerical_gradient(loss_fn_bias, linear.bias.copy())
error = relative_error(grad_b_analytic, grad_b_numeric)
print('Relative Error for bias:', error)
```
...
We can check one parameter in it, such as the weight `fc1.weight` of the first layer. To make numerical gradient checking faster, we only use a very small model:
...
Then use numerical gradients to estimate the gradient of `fc1.weight`:

```{python}
def loss_fn_fc1_weights(weight: np.ndarray) -> float:
...
grad_fc1_W_numeric = numerical_gradient(loss_fn_fc1_weights, model.fc1.weight.copy())
error = relative_error(grad_fc1_W_analytic, grad_fc1_W_numeric)
print('Relative Error for fc1.weight:', error)
```

Similarly, we can also check `fc1.bias`, `fc2.weight`, and `fc2.bias`.
